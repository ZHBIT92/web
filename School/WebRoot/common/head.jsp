<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%
	String path = request.getContextPath();
	String basePath = request.getScheme() + "://"
			+ request.getServerName() + ":" + request.getServerPort()
			+ path + "/";
%>
<%
      String loginUser="";
      if(session.getAttribute("username")!=null){
         loginUser = session.getAttribute("username").toString();
      }
%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<html>
	<head>
		<base href="<%=basePath%>">
		<title>学生信息管理系统主界面</title>
	</head>

	<body>
	<div class="container">
		<nav>
            <a href="__ROOT__"><img id="home" src="Images/1.gif" ></a>
        </nav>
        <div class="container"><p>欢迎你<font color="red"><%=loginUser%></font></p></div>
	<div class="row clearfix">
	<div class="col-md-12 column">		
	<ul class="nav nav-tabs">
		<li ><a class="brand" href="main.jsp"><i class="icon-home"></i>&nbsp;首页</a></li>
		<li><a href="pre?method=studentAdd"><i class="icon-pencil"></i>&nbsp;添加学生信息 </a></li>
		<li><a href="showAllStudents"><i class="icon-book"></i>&nbsp;学生信息维护 </a></li>
		<li><a href="pre?method=preupdatetepwd"><i class=" icon-cog"></i>&nbsp;修改密码</a></li>
		<li><a onclick="check()"><i class="icon-user"></i>&nbsp;退出系统</a></li>
	</ul> 
	</div></div></div>				
	</body>
</html>
